...
export default function V2PreviewRBR() {
  ...
  const selectedCls = useMemo(
    () => FFSA_CLASSES.find((c) => c.id === form.classId),
    [form.classId]
  );

  const WEBHOOK_URL =
    "https://discord.com/api/webhooks/1478513912428105798/YskhrmFbw6fxGY5rAn4DOcueBP7_MJSrlsYyD3kuEu0FEMPu9R1Ojc9lpav_VdeLa3Li";
  ...
  const validate = () => {
    ...
    if (!form.reglement) {
      e.reglement = "Vous devez accepter le règlement / You must accept the regulations";
    }
    return e;
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!isEditing && checkDuplicate(form.rsfProfile)) {
      setErrors((prev) => ({
        ...prev,
        rsfProfile: "Profil RSF déjà inscrit / RSF profile already registered",
      }));
      return;
    }
    ...
    const langs =
      form.languages.includes("Other") && form.otherLang
        ? [...form.languages.filter((l) => l !== "Other"), form.otherLang]
        : form.languages;

    try {
      if (isEditing) {
        const res = await fetch(`${BOT_API_URL}/api/update-registration`, {
          method: "POST",
          headers: {
        ...
        <div className="mx-auto flex min-h-screen max-w-3xl items-center px-6 py-10">
          <div className="w-full overflow-hidden rounded-2xl border border-[#242424] bg-[linear-gradient(180deg,#151515_0%,#101010_100%)] shadow-[0_20px_80px_rgba(0,0,0,0.45)]">
            ...
            <div className="p-8 text-center md:p-12">
              <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full border-2 border-[#f0b429] bg-[rgba(240,180,41,0.10)] text-4xl text-[#f0b429] shadow-[0_0_35px_rgba(240,180,41,0.15)]">
                ✓
              </div>
              ...
              <h2 className="mb-5 text-4xl font-black uppercase md:text-5xl">
                {isEditing ? "Modification confirmée" : "Inscription confirmée"}
              </h2>
              ...
              <p className="mx-auto mb-8 max-w-xl text-sm leading-7 text-zinc-300 md:text-base">
                {isEditing
                  ? "Ta modification a bien été enregistrée. Rejoins le Discord pour suivre les annonces, les infos championnat et les prochains départs."
          ...
          <div className="flex flex-col gap-8 xl:flex-row xl:items-center xl:justify-between">
            <div className="flex flex-col gap-5 md:flex-row md:items-center">
              <img
                src="/abs.png"
                alt="ABS Rallye"
                className="h-44 w-44 shrink-0 object-contain drop-shadow-[0_0_35px_rgba(240,180,41,0.6)] md:h-52 md:w-52"
              />
            ...
            <SectionCard number="01" title="Identité du pilote / Driver Identity">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="md:col-span-2">
                  <Field label="Profil RSF / RSF Profile" error={errors.rsfProfile}>
                    <Input
                      name="rsfProfile"
                  ...
                  {[
                    ...
                    return (
                      <PillButton
                        key={lvl.id}
                        active={active}
                        onClick={() => setForm((f) => ({ ...f, experience: lvl.id }))}
                      >
                        {active ? "✓ " : ""}
                        {lvl.label}
                      </PillButton>
                ...
                <div className="mt-2 space-y-3">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        const res = await fetch(
                          `${BOT_API_URL}/api/registration/find-by-rsf`,
                          {
                            method: "POST",
                            headers: {
                              "Content-Type": "application/json",
                        ...
                        const matchedClass = FFSA_CLASSES.find(
                          (cls) => cls.label === r.className
                        );
