...
const FFSA_CLASSES = [
  ...
  },
  {
    ...
    label: "Rally 3",
    color: "#e67e22",
    cars: ["Renault Clio Rally3", "Ford Fiesta Rally3 evo", "Ford Fiesta Rally3"],
  },
  {
    id: "A8",
    label: "A8",
  ...
  },
  {
    ...
    label: "N4",
    color: "#7d3c98",
    cars: ["Mitsubishi Lancer Evo IX N4", "Subaru Impreza N14 N4", "Seat Leon Cupra R GrpN"],
  },
  {
    id: "R4",
    label: "R4",
    color: "#884ea0",
    cars: ["Mitsubishi Lancer Evo IX R4", "Mitsubishi Lancer Evo X R4", "Subaru Impreza N15 R4"],
  },
  {
    id: "Rally4",
    label: "Rally 4",
    color: "#f39c12",
    cars: ["Peugeot 208 Rally4", "Renault Clio Rally4", "Ford Fiesta Rally4"],
  },
  {
    id: "R3",
    label: "R3",
  ...
  },
  {
    ...
    label: "A6",
    color: "#5b2c8f",
    cars: ["Peugeot 106 Rallye S20 GrpA", "Lada Kalina RC2 GrpA"],
  },
  {
    id: "Rally5",
    label: "Rally 5",
    color: "#d4ac0d",
    cars: ["Renault Clio Rally5"],
  },
  {
    id: "R1",
    label: "R1",
    color: "#1d8348",
    cars: ["Renault Twingo R1", "Citroen DS3 R1"],
  },
];
...
const EXPERIENCE_OPTIONS = [
  { id: "beginner", label: "Débutant / Beginner", discordLabel: "Débutant / Beginner" },
  { id: "intermediate", label: "Intermédiaire / Intermediate", discordLabel: "Intermédiaire / Intermediate" },
  { id: "confirmed", label: "Confirmé / Confirmed", discordLabel: "Confirmé / Confirmed" },
  { id: "expert", label: "Expert / Expert", discordLabel: "Expert / Expert" },
];
...
async function sendDiscordNotification({
      ...
      embeds: [
        {
          ...
          fields: [
            ...
            { name: "Nationalité / Nationality", value: nationality || "Non définie", inline: true },
            {
              ...
              value: languages?.length ? languages.join(", ") : "Non définies",
              inline: true,
            },
            {
              name: "Niveau / Level",
              value: experience || "Non défini",
              inline: true,
            },
            { name: "Classe / Class", value: classe || "Non définie", inline: true },
            { name: "Voiture / Car", value: voiture || "Non définie", inline: true },
            { name: "Profil RSF", value: rsfProfile || "Non défini", inline: false },
          ],
          footer: {
            text: "ABS French Rally League — RBR RSF NGP7",
          },
  ...
  if (!response.ok) {
    ...
    throw new Error(text || "Erreur lors de l'envoi Discord");
  }
}
...
export default function App() {
  ...
  const selectedCls = useMemo(
    () => FFSA_CLASSES.find((c) => c.id === form.classId) || null,
    [form.classId]
  );

  const selectedExperience = useMemo(
    () => EXPERIENCE_OPTIONS.find((lvl) => lvl.id === form.experience) || null,
    [form.experience]
  );
  ...
  const toggleLang = (lang) => {
    const alreadySelected = form.languages.includes(lang);
    setForm((prev) => ({
      ...prev,
  ...
  const validate = () => {
    ...
    if (!form.pseudo.trim()) nextErrors.pseudo = "Requis / Required";
    if (!form.nom.trim()) nextErrors.nom = "Requis / Required";
    if (!form.classId || !form.car) nextErrors.car = "Choisissez une classe et une voiture / Please select a class and a car";
    if (!form.reglement) nextErrors.reglement = "Vous devez accepter le règlement / You must accept the regulations";
  ...
  const handleSubmit = async () => {
    ...
    try {
      const languagesForDiscord = form.languages.includes("Other")
      ...
      await sendDiscordNotification({
        ...
        languages: languagesForDiscord,
        experience: selectedExperience?.discordLabel || form.experience || "Non défini",
    ...
    return (
      <div className="relative min-h-screen overflow-hidden bg-[#090909] text-white">
        ...
        <div className="relative z-10 mx-auto flex min-h-screen max-w-3xl items-center px-6 py-10">
          <div className="w-full overflow-hidden rounded-2xl border border-[#242424] bg-[linear-gradient(180deg,#151515_0%,#101010_100%)] shadow-[0_20px_80px_rgba(0,0,0,0.45)] backdrop-blur-sm">
            <div className="h-1.5 bg-gradient-to-r from-[#f0b429] via-[#ffd55f] to-[#f0b429]" />
            <div className="p-8 text-center md:p-12">
              <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full border-2 border-[#f0b429] bg-[rgba(240,180,41,0.10)] text-4xl text-[#f0b429] shadow-[0_0_35px_rgba(240,180,41,0.20)]">
                ✓
              </div>
              <p className="mb-2 text-xs font-bold uppercase tracking-[4px] text-[#f0b429]">
                ABS French Rally League
              </p>
              <h2 className="mb-5 text-4xl font-black uppercase md:text-5xl">
              ...
              <div className="mx-auto mb-8 grid max-w-2xl gap-3 md:grid-cols-3">
                ...
                <InfoCard
                  label="Classe"
                  value={selectedCls?.label || "—"}
                  accent={selectedCls?.color}
                />
              ...
              <div className="flex flex-wrap items-center justify-center gap-3">
                <a
                  href="https://discord.gg/c3pyhvWjKx"
                  className="rounded-xl bg-[#5865F2] px-6 py-3 text-sm font-bold text-white shadow-lg transition duration-200 hover:-translate-y-1 hover:opacity-90"
                >
                  Rejoindre le Discord
                </a>
                <button
                  className="rounded-xl border border-[#333] px-6 py-3 text-sm text-zinc-300 transition hover:bg-white/5"
                  onClick={() => setSubmitted(false)}
                >
                  Modifier l'inscription
                </button>
                <button
                  className="rounded-xl border border-red-600/40 bg-red-600/10 px-6 py-3 text-sm font-semibold text-red-400 transition hover:bg-red-600/20"
                  onClick={resetForm}
                >
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#090909] text-white">
      ...
      <header className="relative z-10 overflow-hidden border-b border-[#272727] bg-[radial-gradient(circle_at_top_left,rgba(240,180,41,0.16),transparent_28%),radial-gradient(circle_at_bottom_right,rgba(88,101,242,0.12),transparent_24%),linear-gradient(135deg,#111_0%,#151515_60%,#0d0d0d_100%)] px-6 py-10 md:px-8 md:py-12">
        ...
        <div className="absolute -left-12 top-10 h-56 w-56 rounded-full bg-[#f0b429]/20 blur-3xl" />
        <div className="absolute -right-10 bottom-0 h-56 w-56 rounded-full bg-white/8 blur-3xl" />
        <div className="absolute right-6 top-1/2 hidden -translate-y-1/2 select-none text-[110px] font-black tracking-[-5px] text-[rgba(240,180,41,0.05)] xl:block">
          RBR
        </div>
          ...
          <div className="flex flex-col gap-8 xl:flex-row xl:items-center xl:justify-between">
            <div className="flex flex-col gap-5 md:flex-row md:items-center">
              <img
                src="/abs.png"
                alt="ABS Rallye"
                className="h-44 w-44 shrink-0 object-contain drop-shadow-[0_0_35px_rgba(240,180,41,0.6)] transition duration-300 hover:scale-[1.03] md:h-52 md:w-52"
              />
              ...
              <div className="max-w-3xl">
                ...
                <div className="mt-5 flex flex-wrap items-center gap-3">
                  <a
                    href="https://discord.gg/c3pyhvWjKx"
                    className="rounded-xl bg-[#5865F2] px-5 py-3 text-sm font-bold text-white shadow-lg transition duration-200 hover:-translate-y-1 hover:opacity-90"
                  >
                    Rejoindre le Discord
                  </a>
            ...
            <div className="flex items-center justify-center xl:justify-end">
              <img
                src="/afrl.png"
                alt="AFRL"
                className="h-36 object-contain opacity-95 transition duration-300 hover:scale-[1.02] md:h-44"
              />
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_320px]">
          <div className="flex flex-col gap-7">
            <SectionCard number="01" title="Identité du pilote / Driver Identity">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Field label="Nom réel / Real name *" error={errors.nom}>
                  <Input
                    ...
                    value={form.nom}
                    onChange={handleChange}
                    placeholder="Prénom Nom / First Last"
                  />
                </Field>
                ...
                <div className="md:col-span-2">
                  <Field label="Profil RSF / RSF Profile">
                    <Input
                      name="rsfProfile"
                      value={form.rsfProfile}
                      onChange={handleChange}
                      placeholder="rallysimfans.hu/rbr/profile.php?..."
                    />
                  </Field>
                </div>
              </div>
              ...
              <div className="mt-5">
                <p className="mb-2 text-[11px] font-bold uppercase tracking-[2px] text-zinc-500">
                  ...
                  {EXPERIENCE_OPTIONS.map((lvl) => {
                    ...
                    return (
                      <PillButton
                        key={lvl.id}
                        active={active}
                        onClick={() => setForm((prev) => ({ ...prev, experience: lvl.id }))}
                      >
                        {active ? "✓ " : ""}
                        {lvl.label}
                      </PillButton>
            ...
            <SectionCard number="02" title="Classe & Voiture / Class & Car">
              ...
              <div className="flex flex-wrap gap-2.5">
                {FFSA_CLASSES.map((cls) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className="h-2.5 w-2.5 rounded-full transition duration-150 group-hover:scale-125"
                        style={{
              ...
              {currentClass && (
                <div className="mt-5 overflow-hidden rounded-2xl border border-[#252525] bg-[#0d0d0d]">
                  ...
                  >
                    <span className="text-lg font-bold" style={{ color: currentClass.color }}>
                      {currentClass.label}
                    </span>
                  ...
                  <div className="grid gap-2 p-2 md:grid-cols-2">
                    {currentClass.cars.map((car) => {
                      ...
                      return (
                        ...
                        >
                          <span
                            className="flex h-4 w-4 items-center justify-center rounded-full border-2 transition duration-200"
                            style={{
                              borderColor: picked ? currentClass.color : "#444",
                              background: picked ? currentClass.color : "transparent",
                              boxShadow: picked ? `0 0 10px ${currentClass.color}66` : "none",
                            }}
                          >
              ...
              {form.classId && (
                <div
                  className="mt-4 rounded-2xl border bg-black/30 px-4 py-3"
                  style={{ borderColor: selectedCls?.color, color: selectedCls?.color }}
                >
                  🏁 <strong>{selectedCls?.label}</strong>
                  {form.car ? (
                    <span className="ml-2 text-zinc-300">— {form.car}</span>
                  ) : (
                    <span className="ml-2 text-zinc-500">— aucune voiture / no car selected</span>
                  )}
                </div>
              )}
            ...
            <SectionCard number="03" title="Règlement / Regulations">
              <div className="rounded-2xl border border-[#2c2c2c] bg-black/20 p-4">
                <p className="text-sm leading-7 text-zinc-300">
                  📋 Consulte le règlement complet sur le Discord officiel avant de valider ton inscription.
                </p>
                <a
              ...
              <label className="mt-4 flex items-start gap-3 rounded-2xl border border-[#2b2b2b] bg-black/15 p-4 text-zinc-300 transition hover:bg-white/[0.03]">
                ...
                />
                <span>
                  J'ai lu et j'accepte le règlement / I have read and accept the ABS French Rally League regulations
                </span>
              </label>

              {errors.reglement && (
                <p className="mt-2 text-sm text-[#f0b429]">{errors.reglement}</p>
              )}
            </SectionCard>
          </div>

          <aside className="xl:sticky xl:top-6 xl:h-fit">
            <div className="overflow-hidden rounded-2xl border border-[#242424] bg-[linear-gradient(180deg,#151515_0%,#111_100%)] shadow-[0_12px_50px_rgba(0,0,0,0.35)] backdrop-blur-sm">
              <div className="border-b border-[#252525] px-5 py-4">
                <p className="text-xs font-bold uppercase tracking-[3px] text-[#f0b429]">
              ...
              <div className="space-y-3 p-5">
                ...
                <div className="rounded-xl border border-[#2b2b2b] bg-black/20 p-4">
                  <div className="text-[11px] font-bold uppercase tracking-[2px] text-zinc-500">
                    Statut
                  </div>
                  <div className="mt-2 flex items-center gap-2 text-sm">
                    <span
                      className={`inline-block h-2.5 w-2.5 rounded-full ${
                        form.reglement ? "bg-emerald-400" : "bg-zinc-600"
                      }`}
                    />
                ...
                <button
                  ...
                  disabled={isSending}
                  className="mt-2 w-full rounded-xl bg-[#f0b429] px-6 py-4 text-lg font-black uppercase tracking-[2px] text-white shadow-lg transition duration-200 hover:-translate-y-1 hover:brightness-105 active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-70"
                >
                  {isSending ? "ENVOI..." : "VALIDER / SUBMIT →"}
                </button>
              </div>
            </div>
          </aside>
  ...
  return (
    <>
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute left-[5%] top-[8%] h-72 w-72 animate-pulse rounded-full bg-[#f0b429]/12 blur-3xl" />
        <div
          className="absolute right-[10%] top-[20%] h-80 w-80 animate-pulse rounded-full bg-[#5865F2]/10 blur-3xl"
          style={{ animationDuration: "7s" }}
        />
        <div
          className="absolute bottom-[10%] left-[18%] h-64 w-64 animate-pulse rounded-full bg-white/5 blur-3xl"
          style={{ animationDuration: "9s" }}
        />
        <div
          className="absolute bottom-[18%] right-[12%] h-72 w-72 animate-pulse rounded-full bg-[#f0b429]/8 blur-3xl"
      ...
      <style>{`
        @keyframes float {
          ...
          50% { transform: translateY(-10px) translateX(6px); opacity: 0.35; }
...
function SectionCard({ number, title, children }) {
  return (
    <section className="overflow-hidden rounded-2xl border border-[#242424] bg-[linear-gradient(180deg,#141414_0%,#101010_100%)] p-6 shadow-[0_12px_50px_rgba(0,0,0,0.25)] backdrop-blur-sm md:p-7">
...
function Field({ label, children, error }) {
  return (
    <div className="flex flex-col gap-2">
      <label className="text-[11px] font-bold uppercase tracking-[2px] text-zinc-500">
        {label}
      </label>
      {children}
...
function Input(props) {
  return (
    <input
      {...props}
      className="w-full rounded-xl border border-[#2a2a2a] bg-[#1a1a1a] px-4 py-3 text-sm text-white outline-none transition focus:border-[#f0b429] focus:shadow-[0_0_0_3px_rgba(240,180,41,0.12)]"
    />
  );
}
...
function SummaryRow({ label, value, accent }) {
  return (
    <div className="rounded-xl border border-[#2b2b2b] bg-black/20 p-4">
      ...
      <div className="mt-1 text-sm" style={{ color: accent || "#fff" }}>
        {value}
      </div>
    </div>
  );
